...
names.temp<-c("Country", names.temp)
colnames(covid_data)<-names.temp

countries<-factor(covid_data$Country) %>% unique()

summarised_countries<-matrix(, ncol=(ncol(covid_data)-1))

for(i in 1:length(countries)){
  index<-which(covid_data$Country == countries[i])
  summarised_countries<-rbind(summarised_countries, colSums(covid_data[c(index),-c(1)]))
}

summarised_countries<-tbl_df(summarised_countries[-c(1),])
summarised_countries<-cbind(countries, summarised_countries)
colnames(summarised_countries)<-names.temp


save(summarised_countries, file="data/confirmed-cumulative-by-country.Rdata")
